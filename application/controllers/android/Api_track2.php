			...
			if($user_name1!="" && $password1!="")
			{
				...
				if ($query->id!="")
				{
					if ($query->password == $user_password || $user_password==md5($defaultpassword))
					{
						...
						$this->insert_firebase_token($user_session,$device_id,$firebase_token);
					}
					else
					{
...
<?php
	...
	public function insert_firebase_token($user_id,$device_id,$firebase_token)
	{
		$this->db->query("insert into tbl_rider_info set user_id='$user_id',device_id='$device_id',firebase_token='$firebase_token'");
	}
...
<?php
	...
	public function user_location_services($page_type)
	{
		...
		if($device_id!="")
		{
			$this->db->query("update tbl_rider_info set gettime='$gettime',getdate='$getdate',getlatitude='$getlatitude',getlongitude='$getlongitude' where device_id='$device_id'");
		}
	}
}