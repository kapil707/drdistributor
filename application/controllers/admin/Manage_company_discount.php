...
class Manage_company_discount extends CI_Controller {
	...
	{		
		...
		$result =  $this->db->query ("select DISTINCT company_name,compcode from tbl_medicine where company_name Like '$company_name%' or company_name Like '%$company_name' limit 50")->result();
		foreach($result as $row)
		{
			...
			?>
			<li style="list-style: none;margin: 5px;"><a href="javascript:addcompany(<?= $id ?>,'<?= $company_name1 ?>')"><?= $company_name ?> (<?= $compcode ?>)</a></li>
			<?php
		}
		?></ul><?php
	}
